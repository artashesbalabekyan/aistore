...
| Name | Scope | Comment |
| --- | --- | --- |
| [ClusterConfig](https://github.com/artashesbalabekyan/aistore/blob/master/cmn/config.go#L48) | Global | [Named sections](https://github.com/artashesbalabekyan/aistore/blob/master/deploy/dev/local/aisnode_config.sh#L13) containing name-value knobs |
| [LocalConfig](https://github.com/artashesbalabekyan/aistore/blob/master/cmn/config.go#L49) | Local | Allows to override global defaults on a per-node basis |
...
Majority of the configuration knobs can be changed at runtime (and at any time). A few read-only variables are explicitly [marked](https://github.com/artashesbalabekyan/aistore/blob/master/cmn/config.go) in the source; any attempt to modify those at runtime will return "read-only" error message.
