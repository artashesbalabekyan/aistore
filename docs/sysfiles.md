...
> For the most recently updated system filenames and configuration directories, please see [`fname/fname.go`](https://github.com/artashesbalabekyan/aistore/blob/master/cmn/fname/fname.go) source.
...
Thirdly, there are also AIS components and tools, such as [AIS authentication server](https://github.com/artashesbalabekyan/aistore/tree/master/cmd/authn) and [AIS CLI](https://github.com/artashesbalabekyan/aistore/tree/master/cmd/cli). Authentication server, if enabled, creates a sub-directory `.authn` that contains:
